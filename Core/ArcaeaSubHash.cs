...
public class ArcaeaSubHash
{
    ...
    internal void Init()
    {
        ...
        {
            Client.DefaultRequestHeaders.Add("User-Agent", GlobalConfig.Config.ChallengeToken);
            GenerateChallenge = (method, body, path, _) =>
                                    JsonConvert.DeserializeObject<ChallengeApiRoot>(GetString(method, body, path))
                                               ?.Content!;
        }
        else
        {
            GenerateChallenge = (method, body, path, _) =>
                                    JsonConvert.DeserializeObject<TaikariRoot>(GetString(method, body, path))?.Content
                                               ?.Challenge!;
        }
    }
    ...
    public class TaikariRoot
    {
        ...
        [JsonProperty("content")] public TaikariContent? Content { get; set; }
    }
